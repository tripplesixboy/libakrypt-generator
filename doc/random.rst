...
.. c:function:: int int ak_random_create_lcg( ak_random ctx )
...
а каждый октет встечается на периоде одинаковое количество раз.
...
.. c:function:: int int ak_random_create_nlfsr( ak_random ctx )
...
.. c:function:: int int ak_random_create_file( ak_random ctx, const char *name )
...
.. c:function:: int int ak_random_create_winrtl( ak_random ctx )
...
.. c:function:: int ak_random_randomize( ak_random ctx, const ak_pointer ptr, const ssize_t len )
   ...
   Установка внутреннего состояние генератора псевдослучайной последовательности.
