 ...
 int main( void )
{
    ...
    int i = 0, exit_code = EXIT_SUCCESS;
   ...
   /* последний фрагмент снова про хеш, но теперь в свете электронной подписи */
    ...
    for( i = 4000; i < 5000; i++ )
    {
       /* подписываем память, проверяем файл */
        ...
        if( ak_verifykey_verify_file_offset( &vkey, "hello.offset", 0, i, outs ) != ak_true ) {
           printf("sign = %2d No (verify file, start)\n", i );
           exit_code = EXIT_FAILURE;
        }
       /* и наоборот, подписываепм файл - проверяем память */
        ak_signkey_sign_file_offset( &skey, &generator, "hello.offset", 0, i, outs, 64 );
