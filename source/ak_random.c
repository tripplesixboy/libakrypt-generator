...
{
    size_t idx = 0;
    size_t binarySum = 0; /* сумма единиц */

    for( idx = 0; idx < size; idx++ )
    {
        binarySum += ak_hamming_weight_table[data[idx]];
    }

    if( ak_log_get_level() >= ak_log_maximum )
    {
        ak_error_message_fmt( ak_error_ok, __func__,
                                                  "size: %llu, binarySum: %llu", size, binarySum );
    }
   ...
   /* проверяем граничные условия */
    ...
    {
       case 32:
         if((  binarySum >= 102 ) && ( binarySum <= 154 )) return ak_true;
          else return ak_false;

       case 64:
         if((  binarySum >= 219 ) && ( binarySum <= 293 )) return ak_true;
          else return ak_false;

       case 128:
         if((  binarySum >= 460 ) && ( binarySum <= 564 )) return ak_true;
          else return ak_false;
