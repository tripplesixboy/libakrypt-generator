...
/* ----------------------------------------------------------------------------------------------- */
/*! @param wc Указатель, на константные данные, содержащие параметры эллиптической кривой
    @return Функция возвращает числовой идентификатор. Если указатель не верен, то возвращается
    значение undefined_curve.                                                                      */
/* ----------------------------------------------------------------------------------------------- */
 wcurve_id_t ak_wcurve_get_identifier( ak_wcurve wc )
{
  if( wc == NULL ) {
    ak_error_message( ak_error_null_pointer, __func__, "null pointer to elliptic curve");
    return undefined_curve;
  }
  return wc->id;
}

/* ----------------------------------------------------------------------------------------------- */
/*! @param idx Числовой идентификатор эллиптической кривой
    @return Функция возвращает указатель на константные параметры эллиптической кривой.
    Если идентификтаор не верен, то возвращается значение undefined_curve.                         */
/* ----------------------------------------------------------------------------------------------- */
 const struct wcurve *ak_wcurve_get_parameters( wcurve_id_t idx )
{
    ak_oid oid = ak_oid_find_by_mode( wcurve_params );

    do{
      const struct wcurve *wc = oid->data;
      if( wc->id == idx ) return wc;
